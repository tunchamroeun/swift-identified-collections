...
public extension IdentifiedArray {
    ...
    mutating func partition(
        ...
        try _dictionary.partition { _, value in
    ...
    mutating func shuffle<T: RandomNumberGenerator>(using generator: inout T) {
        _dictionary.shuffle(using: &generator)
    }
    ...
    mutating func sort(
        ...
        try _dictionary.sort(by: { try areInIncreasingOrder($0.value, $1.value) })
    }

    @available(iOS 15.0, *)
    @inlinable
    mutating func sort(
        using comparators: [KeyPathComparator<Element>]
    ) where ID == Element.ID, Element: Identifiable {
        self = .init(uniqueElements: sorted(using: comparators))
    }
    ...
    mutating func swapAt(_ i: Int, _ j: Int) {
        _dictionary.swapAt(i, j)
    }
}

public extension IdentifiedArray where Element: Comparable {
